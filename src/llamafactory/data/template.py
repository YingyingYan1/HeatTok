...
from .mm_plugin import get_mm_plugin, Qwen2VLPlugin
...
def get_template_and_fix_tokenizer(tokenizer: "PreTrainedTokenizer", data_args: "DataArguments") -> "Template":
    ...
    # English comment.
    if hasattr(data_args, "use_semantic_patches") and data_args.use_semantic_patches:
        if isinstance(template.mm_plugin, Qwen2VLPlugin):
            template.mm_plugin.use_semantic_patches = data_args.use_semantic_patches
            template.mm_plugin.global_downsample = getattr(data_args, "global_downsample", True)
            template.mm_plugin.semantic_patch_size = getattr(data_args, "semantic_patch_size", 56)
            logger.info_rank0(
                f"Semantic patch mode enabled: patch_size={template.mm_plugin.semantic_patch_size}, "
                f"global_downsample={template.mm_plugin.global_downsample}"
            )
