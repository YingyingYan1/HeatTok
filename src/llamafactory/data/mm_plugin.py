...
class Qwen2VLPlugin(BasePlugin):
    ...
    use_semantic_patches: bool = False  # English comment.
    global_downsample: bool = True  # English comment.
    semantic_patch_size: int = 32  # English comment.
    ...
    def _clone_mm_inputs(self, mm_inputs: dict[str, "torch.Tensor"]) -> dict[str, "torch.Tensor"]:
        cloned = {}
    ...
    def _build_mm_inputs_cache_key(
        self,
        images: list["ImageInput"],
        videos: list["VideoInput"],
        audios: list["AudioInput"],
    ) -> Optional[tuple]:
        if not self.use_semantic_patches or len(images) == 0:
            return None

        image_signatures: list[tuple] = []
        for image in images:
            if not hasattr(image, "size") or not hasattr(image, "tobytes"):
                return None
            image_hash = hashlib.md5(image.tobytes()).hexdigest()[:16]
            image_signatures.append((tuple(image.size), image_hash))

        return (
            "heatok_mm_inputs_v1",
            bool(self.global_downsample),
            int(self.semantic_patch_size),
    ...
    def _get_mm_inputs(
        ...
        mm_inputs = {}
        mm_cache = getattr(processor, "_heatok_mm_inputs_cache", None)
        if not isinstance(mm_cache, dict):
            mm_cache = {}
            setattr(processor, "_heatok_mm_inputs_cache", mm_cache)
        mm_cache_key = None
        if len(images) != 0:
            images = self._regularize_images(
                images,
            ...
            mm_cache_key = self._build_mm_inputs_cache_key(images, videos, audios)
            # English comment.
            # if mm_cache_key is not None and mm_cache_key in mm_cache:
            #     return self._clone_mm_inputs(mm_cache[mm_cache_key])
            
            # English comment.
            if self.use_semantic_patches:
                try:
                    # English comment.
                    import sys
                    import os
                    ...
                    # print(f"[Data Plugin] Processing {len(images)} images with HeatTok/Semantic Patches...")
                    sam_model_raw = getattr(processor, "_sam_model_raw", None)
                    sam_model_dataparallel = getattr(processor, "_sam_model_dataparallel", None)
                    mask_generator = getattr(processor, "_mask_generator", None)
                    
                    # English comment.
                    all_pixel_values = []
                    all_image_grid_thw = []
                    tokens_per_image: list[int] = []
                    global_tokens_per_image = []
                    semantic_tokens_per_image = []
                    all_patch_positions = []
                    all_patch_extents = []
                    all_patch_orientations = []
                    all_gaussian_params = []
                    merge_size = getattr(image_processor, "merge_size", 2)
                    merge_square = merge_size ** 2
                    
                    for image in images:
                        result = process_image_with_semantic_patches(
                            image=image,
                            image_processor=image_processor,
                            sam_model_raw=sam_model_raw,
                            sam_model_dataparallel=sam_model_dataparallel,
                            mask_generator=mask_generator,
                            use_semantic_patches=self.use_semantic_patches,
                            global_downsample=self.global_downsample,
                            semantic_patch_size=self.semantic_patch_size,
                        )
                        pixel_values = result["pixel_values"]
                        image_grid_thw = result["image_grid_thw"]
                        patch_positions = result.get("patch_positions")
                        patch_extents = result.get("patch_extents")
                        patch_orientations = result.get("patch_orientations")
                        gaussian_params = result.get("gaussian_params")
                        ...
                        if patch_extents is not None:
                            all_patch_extents.append(patch_extents)
                        if patch_orientations is not None:
                            all_patch_orientations.append(patch_orientations)
                        if gaussian_params is not None:
                            all_gaussian_params.append(gaussian_params)
                        grid_tokens = (image_grid_thw[:, 0] * image_grid_thw[:, 1] * image_grid_thw[:, 2]) // merge_square
                        tokens_per_image.append(int(grid_tokens.sum().item()))
                        
                        # Extract global and semantic token counts from result
                        if "num_global_tokens" in result and "num_semantic_tokens" in result:
                            global_tokens_per_image.append(result["num_global_tokens"])
                            semantic_tokens_per_image.append(result["num_semantic_tokens"])
                        else:
                            # Fallback: calculate from grid_thw
                            img_grid_thw = image_grid_thw
                            if isinstance(img_grid_thw, torch.Tensor):
                                img_grid_thw = img_grid_thw.numpy()
                            num_global = 0
                            num_semantic = 0
                            if gaussian_params is not None:
                                total_semantic_patches = gaussian_params.shape[0] if len(gaussian_params.shape) > 0 else 0
                                total_grid_rows = img_grid_thw.shape[0]
                                num_global_rows = max(0, total_grid_rows - total_semantic_patches) if self.global_downsample else 0
                                
                                for idx in range(num_global_rows):
                                    grid_t, grid_h, grid_w = img_grid_thw[idx]
                                    num_global += int(grid_t) * max(int(grid_h), 1) * max(int(grid_w), 1)
                                
                                for idx in range(num_global_rows, total_grid_rows):
                                    grid_t, grid_h, grid_w = img_grid_thw[idx]
                                    num_semantic += int(grid_t) * max(int(grid_h), 1) * max(int(grid_w), 1)
                                
                                num_global = num_global // merge_square
                                num_semantic = num_semantic // merge_square
                            else:
                                # No semantic patches, all tokens are global
                                for grid_t, grid_h, grid_w in img_grid_thw:
                                    num_global += int(grid_t) * max(int(grid_h), 1) * max(int(grid_w), 1)
                                num_global = num_global // merge_square
                            
                            global_tokens_per_image.append(num_global)
                            semantic_tokens_per_image.append(num_semantic)
                    
                    # English comment.
                    combined_pixel_values = torch.cat(all_pixel_values, dim=0)
                    combined_image_grid_thw = torch.cat(all_image_grid_thw, dim=0)
                    
                    mm_inputs["pixel_values"] = combined_pixel_values
                    mm_inputs["image_grid_thw"] = combined_image_grid_thw
                    mm_inputs["image_tokens_per_image"] = torch.tensor(tokens_per_image, dtype=torch.long)
                    mm_inputs["global_tokens_per_image"] = torch.tensor(global_tokens_per_image, dtype=torch.long)
                    mm_inputs["semantic_tokens_per_image"] = torch.tensor(semantic_tokens_per_image, dtype=torch.long)
                    if all_patch_positions:
                        combined_patch_positions = torch.cat(all_patch_positions, dim=0)
                        if combined_patch_positions.shape[0] != combined_pixel_values.shape[0]:
                            raise ValueError(
                                "image_patch_positions count does not match pixel_values count, "
                                f"positions={combined_patch_positions.shape[0]}, pixels={combined_pixel_values.shape[0]}"
                            )
                        mm_inputs["image_patch_positions"] = combined_patch_positions
                    if all_patch_extents:
                        combined_patch_extents = torch.cat(all_patch_extents, dim=0)
                        if combined_patch_extents.shape[0] != combined_pixel_values.shape[0]:
                            raise ValueError(
                                "image_patch_extents count does not match pixel_values count, "
                                f"extents={combined_patch_extents.shape[0]}, pixels={combined_pixel_values.shape[0]}"
                            )
                        mm_inputs["image_patch_extents"] = combined_patch_extents
                    if all_patch_orientations:
                        combined_patch_orientations = torch.cat(all_patch_orientations, dim=0)
                        if combined_patch_orientations.shape[0] != combined_pixel_values.shape[0]:
                            raise ValueError(
                                "image_patch_orientations count does not match pixel_values count, "
                                f"orientations={combined_patch_orientations.shape[0]}, pixels={combined_pixel_values.shape[0]}"
                            )
                        mm_inputs["image_patch_orientations"] = combined_patch_orientations
                    ...
                    logger.warning_rank0(f"Semantic patch processing failed; using original path: {e}")
        ...
        # English comment.
        # if mm_cache_key is not None:
        #     mm_cache[mm_cache_key] = self._clone_mm_inputs(mm_inputs)
        return mm_inputs
    ...
    def process_messages(
        ...
        image_tokens_per_image = None
        global_tokens_per_image = None
        semantic_tokens_per_image = None
        if self.expand_mm_tokens:
            mm_inputs = self._get_mm_inputs(images, videos, audios, processor)
            image_grid_thw = mm_inputs.get("image_grid_thw", [])
            video_grid_thw = mm_inputs.get("video_grid_thw", [])
            image_tokens_per_image = mm_inputs.get("image_tokens_per_image")
            global_tokens_per_image = mm_inputs.get("global_tokens_per_image")
            semantic_tokens_per_image = mm_inputs.get("semantic_tokens_per_image")
            
            # English comment.
            if image_tokens_per_image is not None and len(images) > 0:
                import os
                if os.getenv("HEATTOK_SEQ_DEBUG"):
                    print(f"[process_messages] image_tokens_per_image: {image_tokens_per_image.tolist()}")
        else:
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                if image_tokens_per_image is not None:
                    image_seqlen = int(image_tokens_per_image[num_image_tokens].item())
                else:
                ...
                split_total = -1
                if (
                    self.use_semantic_patches
                    and global_tokens_per_image is not None
                    and semantic_tokens_per_image is not None
                ):
                    num_global = int(global_tokens_per_image[num_image_tokens].item())
                    num_semantic = int(semantic_tokens_per_image[num_image_tokens].item())
                    split_total = num_global + num_semantic
                    use_split = split_total == int(image_seqlen)

                if use_split:
                    replacement = (
                        f"{self.vision_bos_token}{self.image_token * num_global}{self.vision_eos_token}"
                        f"{self.vision_bos_token}{self.image_token * num_semantic}{self.vision_eos_token}"
                    )
                else:
                    replacement = f"{self.vision_bos_token}{self.image_token * image_seqlen}{self.vision_eos_token}"

                if (
                    os.environ.get("HEATTOK_SEQ_DEBUG", "0").strip().lower() in ("1", "true")
                    and self.use_semantic_patches
                    and global_tokens_per_image is not None
                    and semantic_tokens_per_image is not None
                    and not use_split
                ):
                    print(
                        "[HeatTok Seq] fallback to single segment because split mismatch: "
                        f"global={num_global}, semantic={num_semantic}, split_total={split_total}, "
                        f"image_tokens_per_image={int(image_seqlen)}"
                    )
                if os.environ.get("HEATTOK_SEQ_DEBUG", "0").strip().lower() in ("1", "true"):
                    _seq_preview = replacement if len(replacement) <= 240 else (replacement[:240] + "...<truncated>")
                    print(
                        "[HeatTok Seq] image replacement: "
                        f"has_bos={self.vision_bos_token in replacement}, "
                        f"has_eos={self.vision_eos_token in replacement}, "
                        f"len={len(replacement)}, preview={_seq_preview}"
                    )
