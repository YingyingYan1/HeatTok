...
class DataArguments:
    ...
    global_downsample: bool = field(
        default=True,
        metadata={"help": "Whether to add global downsampled image patches when using semantic patches."},
    )
    semantic_patch_size: int = field(
        default=32,
        metadata={"help": "Size of semantic patches (64 or 32). 64 produces 4 tokens per patch, 32 produces 1 token per patch. Default is 32 for 1 token per patch."},
    )
