...
LORA_MODEL_ID = _PROJECT_ROOT / "outputs/EarthVQA-qwen2.5vl-7B-heatok/checkpoint-7200"
HF_MODEL_ID = LORA_MODEL_ID
DATA_JSON = _PROJECT_ROOT / "data/EarthVal_QA.json"
IMAGES_DIR = _PROJECT_ROOT / "EarthVQA/Val/images_png"
OUT_DIR = _PROJECT_ROOT / "val/earthvqa-qwen2.5vl-7B-heatok-checkpoint-7200-4epoch"
BATCH_SIZE = 1
MAX_WORKERS = 1
MAX_NEW_TOKENS = 64          
TEMPERATURE = 0.0         
RETRIES = 1           
BACKOFF = 1.5           
TASK_MODE = "generic"  # EarthVQA mixed question types
ENFORCE_INTEGER_ONLY_PROMPT = (TASK_MODE == "quantity")
ENFORCE_CATEGORY_ONLY_PROMPT = (TASK_MODE == "category")

# HeatTok semantic patch / EarthVQA settings (aligned with training yaml + semantic_patch_utils)
SEMANTIC_CACHE_DIR = _PROJECT_ROOT / "src/semantic_patch_cache"
...
GLOBAL_DOWNSAMPLE_DIVISOR = 8  # gd1: full-resolution global branch (not gd8)
SEMANTIC_PATCH_SIZE = 28
SEMANTIC_IMAGE_MAX_PIXELS = 1024 * 1024  # 1048576, EarthVQA training resolution
SEMANTIC_IMAGE_MIN_PIXELS = 32 * 32

# EarthVQA heat-diffusion defaults (same as src/llamafactory/utils/semantic_patch_utils.py)
EARTHVQA_MERGE_PARAMS = {
    "sam_morph_kernel": 5,
...
# FastSAM (EarthVQA default backend in SEMANTIC_CACHE.md)
SAM_BACKEND = "fastsam"
...
def _semantic_cache_candidates(image_hash: str) -> List[pathlib.Path]:
    ...
    names = [
        ...
        f"{image_hash}_g{g}_s{s}.pt",  # legacy cache name
    ]
...
def profile_inference_steps(data: List[Dict]) -> None:
    ...
    on NUM_PROFILE_IMAGES images at EarthVQA resolution (1024x1024).
    """
    ...
    profile_items = []
    for qa in data:
        img_path = pathlib.Path(str(qa.get("image_id", "")))
    ...
    print(f"[Profile] Running pipeline on {len(profile_items)} images ({int(math.sqrt(SEMANTIC_IMAGE_MAX_PIXELS))}x{int(math.sqrt(SEMANTIC_IMAGE_MAX_PIXELS))})...")
    merge_params = dict(EARTHVQA_MERGE_PARAMS)
    ...
    for img_path, question in profile_items:
        ...
        w, h = image.size
        if w * h > SEMANTIC_IMAGE_MAX_PIXELS:
            image = _preprocess_image_for_cache(img_path)
        elif w * h < SEMANTIC_IMAGE_MIN_PIXELS:
            image = _preprocess_image_for_cache(img_path)
        try:
...
def _resolve_image_path(image_id: str) -> pathlib.Path:
    """Resolve EarthVQA image path (relative to project root or absolute)."""
    img_path = pathlib.Path(image_id)
    if img_path.is_absolute():
        return img_path
    return (_PROJECT_ROOT / img_path).resolve()
...
def _process_one(qa_item: Dict) -> Record:
    ...
    q_type = str(qa_item.get("type") or qa_item.get("question_type") or "generic")

    # EarthVQA: image_id is relative to project root, e.g. EarthVQA/Val/images_png/2966.png
...
def main():
    ...
    # Require CUDA and enough GPUs
    if torch.cuda.is_available():
        # Log GPUs visible to PyTorch
        ...
        if not visible_gpus:
             print("Error: CUDA is available but PyTorch sees no GPUs.")
             return
        if torch.cuda.device_count() < REQUIRED_NUM_GPUS:
            print(
                f"Error: only {torch.cuda.device_count()} GPU(s) visible, "
                f"but {REQUIRED_NUM_GPUS} required (CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', '')})"
            )
             return
    else:
    ...
    # Load evaluation data (EarthVal_QA.json)
    try:
        data = json.loads(DATA_JSON.read_text(encoding="utf-8"))
        if not isinstance(data, list):
            print(f"Error: {DATA_JSON} must be a JSON array of QA items.")
            return
    ...
    # [Profiling] Run step timing on NUM_PROFILE_IMAGES (1024x1024), then exit
    ...
    for qa_item in pbar:
        try:
            ...
            print(f"[Rank {RANK}] Failed on {img_id_for_error}, skipping: {e}")
    
    print(f"[Rank {RANK}] Finished {len(all_results)} items.")

    # --- Save per-rank predictions ---
    if not all_results:
        print(f"[Rank {RANK}] No results to save, exiting.")
        return

    rank_json_path = OUT_DIR / f"predictions_rank{RANK}.json"
    print(f"[Rank {RANK}] Saving rank predictions to {rank_json_path} ...")
    try:
    ...
    # Multi-process sync before merge
    if WORLD_SIZE > 1 and torch.distributed.is_available():
        try:
            ...
            print(f"[Rank {RANK}] Distributed barrier failed (continuing): {e}")
    ...
    for r in range(WORLD_SIZE):
        ...
        if not part_path.exists():
            print(f"[Rank 0] Warning: missing rank file {part_path}")
            continue
        try:
    ...
    print(f"[Rank 0] Writing merged predictions to {json_path} ...")
    try:
    ...
    print(f"[Rank 0] Writing results CSV to {csv_path} ...")
    ...
    print(f"[Rank 0] Done. Output dir: {OUT_DIR}")
